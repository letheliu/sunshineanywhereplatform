...
//######################教育组件-权限较验部分##########################
	...
	$sql = "select 缴纳时间 as 缴纳日期,Date_Format(缴纳时间,'%Y-%m') as 缴纳月份,机构名称,人员编号,姓名,sum(缴纳金额) as 缴纳金额 from edu_leaguefee group by 缴纳日期,机构名称,人员编号";
	ShowData();
...
<?php
	...
	{
		global $db;
		$sql = "select distinct Date_Format(缴纳时间,'%Y-%m') as 缴纳月份 from edu_leaguefee  order by 缴纳月份 desc ";
		$rs = $db->Execute($sql);
		$rs_a = $rs->GetArray();
		...
		print "<H2 align=center>团费收支信息统计</H2>";
		...
		print "<Td colspan=35 height=22>团费收支信息统计 $title</Td></tr>";
	...
	function By_Organization($by_organization_day,$by_organization_month,$by_person_day,$by_person_month,$total_day,$Month_Pure,$Year_Pure)
	{
		...
		global $db;
		//判断总共有多少机构在本月有过操作
		$sql = "select distinct 机构名称 from edu_leaguefee where Date_Format(缴纳时间,'%Y-%m')= '".$Year_Pure."-".$Month_Pure."'";
		$rs = $db->Execute($sql);
		$rs_a = $rs->GetArray();
				...
				for($j=0;$j<sizeof($编号);$j++)
				{
					for($day=1;$day<=$total_day;$day++)
					{
						 ...
						 if($day==1)
						{
							...
							$姓名 = returntablefield("edu_leaguemember","编号",$编号[$j],"姓名");
