...
function 增加对查询日期快捷方式的支持($目标判断字段)					{
...
function 修改时同步姓名数据($学号,$姓名) {
  global $_GET,$_POST,$db;
  $sql = "update bukao_student set 姓名='$姓名' where 学号='$学号'";
  $db->Execute($sql);
  ...
  //$db->Execute($sql);
  //$sql = "update newedu_gerenchufen set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_gongyihuodong set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_keyanxiangmu set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_luquxuesheng set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_peixunjingli set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_shehuishijian set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_tijianjilu set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_xianxuejilu set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_xiaowaijiangcheng set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_xiaowaishetuan set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_xueshengjingli set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_zhengshuguanli set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
  //$sql = "update newedu_zhuxuedaikuan set 姓名='$姓名' where 学号='$学号'";
  //$db->Execute($sql);
...
function 修改时同步学生的班级名称数据($学号,$班级名称)								{
  ...
  $sql = "update bukao_student set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  ...
  $db->Execute($sql);
  $sql = "update newedu_gerenchufen set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_gongyihuodong set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_keyanxiangmu set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_peixunjingli set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_shehuishijian set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_tijianjilu set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_xianxuejilu set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_xiaowaijiangcheng set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_xiaowaishetuan set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_xueshengjingli set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_zhengshuguanli set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
  $sql = "update newedu_zhuxuedaikuan set 班级='$班级名称' where 学号='$学号'";
  $db->Execute($sql);
...
function returnBanjiCourseList($ClassCode,$returnCurXueQiIndex,$考试名称='')				{
	...
	if($SCHOOL_MODEL_TEXT=="4")			{
		...
		$sql = "select distinct 课程名称 from edu_planexec where 开课学期='$returnCurXueQiIndex' and 班级名称='$ClassCode' and 课程名称!='' order by 课程名称";
	}
	//print $sql;exit;
...
//返回应交学费数组-中小学部分
function ReturnOfficeFeeMiddleSchool($NJ='2006',$XN='2006-2007',$适用收费标准='普通')		{
	global $db;
	// and 学年='$XN'
	if($XN=='设置所有学年学费'||$XN=='设置所有学期学费')		{
...
function 学生批量缴费自动扣款函数($学号,$姓名,$班级,$专业,$年级,$费用金额,$收费单号='',$收费日期='',$发票较验码='')				{
	...
	$专业代码		= returntablefield("edu_zhuanye","专业名称",$专业,"专业代码");

	$sql = "select * from edu_zhuanyeshoufei where 专业代码='$专业代码' and 年级='$年级' and 适用收费标准='$适用收费标准' order by 学年,收费项目代码,收费项目名称";
	//print $sql."<BR>";
	$rs = $db->Execute($sql);
    $rsX_a = $rs->GetArray();
	$NewArray = array();
	...
	for($iX=0;$iX<sizeof($rsX_a);$iX++)		{
		...
		if($上次己缴收费金额<$收费标准)					{
			...
			if($实际金额>0)							{
				...
				//print "本次学费:".$sql."<BR>";
				//print $sql."<BR>";
				//$摘要RETURN = "存入:".$姓名."(".$学号.")".$学年."学年".$收费项目名称."".$实际金额."元";
			}
			else	{
				//$摘要RETURN = "不欠费:".$姓名."(".$学号.")".$学年;
			}
			if($break)  break;
		}//如果没有记录,$上次己缴收费金额 为0
