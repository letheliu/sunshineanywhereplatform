<?
	...
	if($_GET['action']=='view_default'){
          ...
          $ID = $_GET['编号'];
		  $sql = "select * from crm_yanchifukuan_sq where 编号='$ID'";
		  $rs = $db->Execute($sql);
		  $rs_a = $rs->GetArray();
	...
	$parse_filename		=	'crm_yanchifukuan_bmsq';
	...
	if($_GET['action']==''||$_GET['action']=='init_default'||$_GET['action']=='init_customer')		{
			...
			说明：<BR>
			&nbsp;&nbsp;&nbsp;1、审核和作废是有总经理进行操作的，只有总经理有审核和作废的权限。<BR>
			&nbsp;&nbsp;&nbsp;2、单子作废后，系统不允许再对其进行操作,审核人和作废人的信息会显示在备注栏里面。<BR>
			</font></td></table>";
			print $PrintText;
		}
