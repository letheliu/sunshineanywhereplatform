...
if($_GET['action']=="add_default_data")						{
	...
	if($NUM==0&&$角色名称!='')					{
		...
		print_infor("您输入的角色名称己经存在,请换用其它名称!",'',"location='?'",'?');
		exit;
	}
...
if($_GET['action2']=="")		{
	...
	$返回 = "/general/system/user_priv/".$_GET['action2'];
}
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big"><img src="images/notify_new.gif" WIDTH="22" HEIGHT="20" align="absmiddle"><span class="big3"> <?=$_GET['PRIV_NAME']?>权限 - <?=$显示文本?> </span>&nbsp;&nbsp;
    <div id="OP_BTN" style="width:150px;top:5px;right:20px;position:absolute;">
     <form name="form1" method="post" action="?<?=base64_encode("action=SettingPrivData&USER_PRIV=$USER_PRIV&PRIV_NAME=$PRIV_NAME&action2=".$_GET['action2']."&FF=FF")?>">
      ...
      <input type="button" value="返回" class="BigButton" onclick="location='<?=$返回?>'">
     </form>
    </div>
    </td>
  </tr>
	...
	<table class="TableBlock" align="center">
     <tr class="TableContent" title="$PARENT_MENU_NAME">
      <td nowrap>
        ...
        <img src="/images/menu/@EDU.gif" width=19 height=17> <b>$PARENT_MENU_NAME</b>
      </td>
     </tr>
EOF;
...
echo <<<EOF
		<tr title="$GROUP_ONE_NAME">
          ...
          <input type="checkbox" name="{$PARENT_MENU_CODE}" value="$ValueTemp" $CheckedText ><img src="/images/menu/EDU.gif" width=19 height=17> $GROUP_ONE_NAME
          </td>
        </tr>
EOF;
...
echo <<<EOF
		<tr title="$GROUP_ONE_NAME">
          ...
          <img src="/images/menu/EDU.gif" width=19 height=17> $GROUP_TWO_NAME
          </td>
        </tr>
EOF;
	}
...
echo <<<EOF
		<tr title="$GROUP_ONE_NAME">
          <td class="TableData" nowrap>
          <input type="checkbox" name="{$PARENT_MENU_CODE}" value="$ValueTemp"  $CheckedText>
          <img src="/images/menu/EDU.gif" width=19 height=17> $GROUP_ONE_NAME
          </td>
        </tr>
EOF;
}
