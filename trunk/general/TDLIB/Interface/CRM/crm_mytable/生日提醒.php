...
if ( $module_func_id == "" || find_id( $user_func_id_str, $module_func_id ) )
{
				...
				$query = "SELECT 客户名称,客户生日,第一联系人 from crm_customer where 1=1 order by SUBSTRING(客户生日,6,5),客户名称 ASC";
				$rs = $db->Execute($query);
				$ROW = $rs->GetArray();
				...
				if ( 0 < $COUNT1 )
				{
								$MODULE_BODY .= "今日客户生日：".$PERSON_STR1."，生日快乐!<br> ";
				}
				if ( 0 < $COUNT2 )
				{
								$MODULE_BODY .= "近期客户生日：".$PERSON_STR2;
				}
				if ( $COUNT1 == 0 && $COUNT2 == 0 )
				{
								$MODULE_BODY .= "<li>近期客户无生日</li>";
				}
}
?>
