...
&nbsp;<font color="green">当前月份:<?=$当前月份;?></font>&nbsp;<BR>
&nbsp;<font color="blue">当前薪酬项目:<?=$费用名称;?>
</font></td>
</tr>
<tr class="TableHeader">
<td>
...
?>
<input type="hidden" name="当前月份" value=<?=$当前月份;?> />
<input type="hidden" name="当前学期" value=<?=$当前学期;?> />
<input type="hidden" name="费用类别" value=<?=$费用类别;?> />

<input type="hidden" name="费用名称" value=<?=$费用名称;?> />
</td>
</tr>
<tr class="TableData">
<td align="center">
...
<tr class="TableData">
<td><font color="red">&nbsp;当前学期:<?=$当前学期;?></font>&nbsp;<BR>&nbsp;<font color="green">当前月份:第<?=$当前月份;?>月</font>&nbsp;<BR>
&nbsp;<font color="blue">费用名称:<?=$费用名称;?>(<?=$费用类别;?>)</font>
<BR>&nbsp;<font color="blue">己有人员:<?=$己有人员;?></font>
</td>
</tr>
<tr class="TableHeader">
<td>
...
<?
	...
	if($_GET['action']=="从上月获取数据"&&$_GET['当前月份']!="")								{
		...
		print_infor("你的数据己经初始化完成.",'',"location='?当前月份=".$当前月份."'");
	...
	if(sizeof($rs_a)==0||sizeof($rs_a本月)>0||$当前月份!=$m)		{
		//没有记录,不能获取数据
		$disabled = "disabled readonly title='本月己经有数据或上月无数据,不能从上月获取信息'";
	}
...
<input name='从上月获取数据' value='从第<?=($当前月份-1)?>月获取薪酬数据' class=SmallButton type=Button OnClick="location='?当前月份=<?=$当前月份?>&action=从上月获取数据'" <?=$disabled?>>
<input name='计算上个月薪酬' value='计算<?=($当前月份-1)?>月薪酬数据' class=SmallButton type=Button OnClick="location='?当前月份=<?=$当前月份?>&action=计算上个月薪酬'" <?=$able?>>
</td>
</tr>
