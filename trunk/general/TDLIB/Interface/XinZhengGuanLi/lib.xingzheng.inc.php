...
$datetime = date("Y-m-d");
$sql = "select * from edu_schoolcalendar where 开始时间<='$datetime' and 结束时间>='$datetime'";
...
function 执行插入某人某天考勤信息($学期,$人员,$人员用户名,$考勤日期,$班次='')		{
	...
	$sql = "select distinct 班次名称 from edu_xingzheng_paiban
			where 排班人员 like '%$人员用户名%' and 考勤日期='$考勤日期' and 学期名称='$学期'";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	...
	//过滤所得到的班次列表,进行过滤处理,在此己经插入到考勤数据表中
	for($i=0;$i<sizeof($rs_a);$i++)				{
		$班次名称 = $rs_a[$i]['班次名称'];
...
//人员调班过滤,调班过滤方法:先删除己经存在的数据,然后得到新的上班时间和班次信息,重新赋值上班时间和班次
