...
$sql = "select distinct 所属部门 from booksset";
$rs = $db->Execute($sql);
$rsX_a = $rs->GetArray();
...
for($i=0;$i<sizeof($rsX_a);$i++)			{
	...
	$rs_a = $rs->GetArray();
	if($部门名称!="")			{
		print "<tr class=TableData><td>&nbsp;".$部门名称."</td><td><a href=\"../../../system/netdisk/set_priv_fixedasset/set_user.php?".base64_encode("FileNameSELF=".$PHP_SELF_SELF."&FileName=".$PHP_SELF."&ModuleName=".$部门名称."")."\">&nbsp;编辑权限</a></td>
