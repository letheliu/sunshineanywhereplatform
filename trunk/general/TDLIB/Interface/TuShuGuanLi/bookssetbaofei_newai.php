<? 
	...
	if($_POST['批准人']!=""&&$_POST['报废申请人']!="")	{	
		...
		$sql = "update booksset set 使用人员='$报废申请人',所属状态='图书己报废' where 图书编号='$图书编号'";
		$db->Execute($sql);
		//print $sql;
	}
	else			{
	...
	if(0)		{
		//$sql = "update booksset set 所属状态='购置已分配' where 所属状态='图书己报废'";
		//$db->Execute($sql);
		...
		for($i=0;$i<sizeof($rs_a);$i++)		{
			...
			$db->Execute($sql);
			//print $sql."<BR>";;
			$sql = "update booksset set 所属状态='图书己报废' where 图书编号='$图书编号'";
			$db->Execute($sql);
			//print $sql."<BR>";;
		}
	}
