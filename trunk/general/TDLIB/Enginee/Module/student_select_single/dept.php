	...
	if(key!=kword.value && kword.value!="")
	{
     key=kword.value;
	   ...
	   if($('search_icon').src.indexOf("/images/quicksearch.gif")>=0)
	   {
	   	   ...
	   	   $('search_icon').onclick=function(){kword.value='按学生名称或学号搜索...';$('search_icon').src="/images/quicksearch.gif";$('search_icon').title="";$('search_icon').onclick=null;};
	   }
  }
...
<div style="border:1px solid #000000;margin-left:2px;background:#FFFFFF;">
  <input type="text" id="kword" name="kword" value="按学生名称或学号搜索..." onfocus="ctroltime=setTimeout(CheckSend,100);" onblur="clearTimeout(ctroltime);if(this.value=='')this.value='按学生名称或学号搜索...';" class="SmallInput" style="border:0px; color:#A0A0A0;width:145px;"><img id="search_icon" src="/images/quicksearch.gif" align=absmiddle style="cursor:pointer;">
</div>
...
<?
	...
	/*
	if($sunshine_teacher_banzhuren_class!=""&&$sunshine_teacher_class!="")		{
		$sunshine_teacher_classTEXT  = $sunshine_teacher_class.",".$sunshine_teacher_banzhuren_class;
	}
	else if($sunshine_teacher_banzhuren_class==""&&$sunshine_teacher_class!="")		{
		$sunshine_teacher_classTEXT  = $sunshine_teacher_class;
	}
	else if($sunshine_teacher_banzhuren_class!=""&&$sunshine_teacher_class=="")		{
	...
	for($i=0;$i<sizeof($sunshine_teacher_class_array);$i++)		{
		$FieldValue = $sunshine_teacher_class_array[$i];
		print "
		<tr class=TableData>
	...
	print "
		<tr class=TableData>
		<td align=center onclick=\"javascript:parent.user.location='user.php?TO_ID=".$_GET['TO_ID']."&TO_NAME=".$_GET['TO_NAME']."&FORM_NAME=".$_GET['FORM_NAME']."&action2=按班委选择&XX=XX';\" style=\"cursor:pointer\">按班委选择</td>
		</tr>
		";
	print "
		<tr class=TableData>
		<td align=center onclick=\"javascript:parent.user.location='user.php?TO_ID=".$_GET['TO_ID']."&TO_NAME=".$_GET['TO_NAME']."&FORM_NAME=".$_GET['FORM_NAME']."&action2=按宿舍长选择&XX=XX';\" style=\"cursor:pointer\">按宿舍长选择</td>
		</tr>
		";
