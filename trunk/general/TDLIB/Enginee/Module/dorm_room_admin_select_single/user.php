...
if($_GET['房间名称']!="")		{
	...
	$sql = "select 床位号 from edu_student where 学生宿舍='".$房间名称."' and 床位号>'0'  and 学生状态='正常状态'";
	...
	$sql = "select 床位号 from edu_newstudent where 学生宿舍='".$房间名称."' and 床位号>'0'  and 是否录取='1' and 是否报道='1'";
	$rs = $db->CacheExecute(150,$sql);
	$rs_a = $rs->GetArray();
	for($i=0;$i<sizeof($rs_a);$i++)			{
		$已经用过的床位[] = $rs_a[$i]['床位号'];
	}
	$余下床位号 = @array_diff($所有床位数组,$已经用过的床位);
