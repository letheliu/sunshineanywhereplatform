...
function newai_search($fields)			{
	...
	for($i=0;$i<sizeof($showlistfieldlistArray);$i++)				{
		...
		switch($fieldfilter)		{
			...
			case 'date':
				...
				print "<INPUT class=SmallInput size=10  name=\"".$listIndexName."_开始时间\" value=\"$BEGIN_DATE\">
				<input type=\"button\" value=\"选\" class=\"SmallInput\" style=\"width:25px\" onclick=\"td_calendar('../../Framework/sms_index/calendar_begin.php?datetime=".$listIndexName."_开始时间');\" name=\"button\">&nbsp;
				";
				print "<INPUT class=SmallInput size=10  name=\"".$listIndexName."_结束时间\" value=\"$END_DATE\">
				<input type=\"button\" value=\"选\" class=\"SmallInput\" style=\"width:25px\" onclick=\"td_calendar('../../Framework/sms_index/calendar_begin.php?datetime=".$listIndexName."_结束时间');\" name=\"button\">&nbsp;
				";
				break;
			case 'number':
				$sql = "select max(".$listIndexName.") AS 最大值,min(".$listIndexName.") AS 最小值 from $tablename";
			...
			case 'input':
			default:
				print "<input type=\"text\" title='模糊查询' name=\"".$listIndexName."\" class=\"SmallInput\" size=\"10\" maxlength=\"100\" value=\"".$_GET[$listIndexName]."\">";
				break;
	...
	//得到初始经状态下面的所有字段的列表
	$showlistfieldlist_init = $file_ini['init_default']['showlistfieldlist'];
	...
	print "<script>function ResultExportXLS(LOCALURL)	{
		 ...
		 <tr>
			...
			&nbsp;&nbsp;
			<input class=\"SmallButton\" value=\"快速查询\" type=\"submit\" title=\"快速查询\" name=\"快速查询\">&nbsp;
			<input class=\"SmallButton\" value=\"快速返回\" type=\"button\" title=\"快速返回\" name=\"快速返回\" onClick=\"location='?'\">&nbsp;
			<input class=\"SmallButton\" value=\"结果导出\" type=\"button\" title=\"结果导出\" name=\"结果导出\" onClick=\"ResultExportXLS('$return')\">&nbsp;
