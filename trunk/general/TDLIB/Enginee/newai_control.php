...
switch($action_type)		{
	...
	case 'init':
		...
		session_register("指定人员用户名");

		if($_GET['指定人员用户名']!=""&&$_SESSION['LOGIN_USER_ID']!="admin")						{
			$_SESSION['指定人员用户名'] = $_GET['指定人员用户名'];
		}
		else	{
			$_SESSION['指定人员用户名'] = '';

		}
		if($_SESSION['指定人员用户名']!=""&&$_SESSION['LOGIN_USER_ID']!="admin")		{
			$action_model = '';
			$row_element = '';
			$bottom_element = '';
			$_GET['系统说明'] = " 1 看到此变量,表示GET里面的指定教师用户名等变量由系统指定,所有系统只能有查看权限; 2 同时在INI文件由系统进行对action_model row_element bottom_element等几个变量进行重新定义";

		}

		if($_GET['指定班号']!=""&&$_SESSION['LOGIN_USER_ID']!="admin")						{
		...
		if($_SESSION['指定班号']!=""&&$_SESSION['LOGIN_USER_ID']!="admin")		{
		...
		if($SYSTEM_MODE=="1"&&$FileDirName!="PGSQL"&&in_array('1',$总角色数组))			{
			...
			$MODULE_ARRAY_NAME = array_pop($SCRIPT_FILENAME_ARRAY);
			print "<BR><div align=center><a href=\"../../Development/main.php?MakeSystemModel=$MODULE_ARRAY_NAME&Tablename=$tablename&FileIniname=$parse_filename\" target=_blank><font color=red>点击配置</font></a><BR>
