...
class ADODB_postgres7 extends ADODB_postgres64 {
	...
	*/
	...
	function _query($sql,$inputarr=false)
	{
		...
		for($i=0;$i<sizeof($过滤变量数组);$i++)		{
			$过滤变量名称 = trim($过滤变量数组[$i]);
			$sql = ereg_replace(" $过滤变量名称 "," \"$过滤变量名称\" ",$sql);
			$sql = ereg_replace(" $过滤变量名称="," \"$过滤变量名称\"=",$sql);
			if(substr($sql,-5)==" $过滤变量名称")	$sql = ereg_replace(" $过滤变量名称"," \"$过滤变量名称\" ",$sql);
		...
		if ($rez && pg_numfields($rez) <= 0) {
			...
			if(strlen($sql)>6&&$SYSTEM_MODE==1&&!$rez) {
				...
				print "ERROR SQL: ".$sql.$IsFOrS."<BR>DRIVER FILE: adodb-postgres7.inc.php<BR>".pg_last_error();;
				exit;
			}
		}
		return $rez;
