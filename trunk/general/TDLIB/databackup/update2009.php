...
if($_GET['action']=="")                                {
print "<link rel=stylesheet href=\"/theme/3/style.css\"><body class=bodycolor><title>通达OA教育管理组件-更新程序</title>";
...
for($i=0;$i<sizeof($file);$i++)                {
    ...
    if($FILE_EXEC=="1")                        {
        ...
        if($PacketNameUpdate[$fileName]=="1")        {
            $ReDownload = "<a href=?".base64_encode("action=ExecRemoteFileServer&FileName=$fileName&RemoteHostName=".$_GET['RemoteHostName']."")."><font color=003399 title='己安装,点击重新安装'>己安装,点击重新安装</font></a>";
        }
        else    {
    ...
    if(file_exists($key.$FileName.".sql"))    {
        ...
        for($i=0;$i<sizeof($FileArray);$i++)        {
			...
			if($sql=="SET USER PRIV ALL")		{
				...
				$sql = "update user_priv set FUNC_ID_STR='$NewText' where USER_PRIV='1'";
				$rs = $db->Execute($sql);
				//print $sql;exit;
			}
			else	{
