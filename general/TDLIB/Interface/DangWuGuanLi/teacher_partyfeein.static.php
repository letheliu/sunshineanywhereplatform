<?php
	...
	CheckSystemPrivate("人力资源-党员管理-缴纳统计");
	page_css("党费缴纳统计");
	...
	$sql = "select 缴纳时间 as 缴纳日期,Date_Format(缴纳时间,'%Y-%m') as 缴纳月份,机构名称,人员编号,姓名,sum(缴纳金额) as 缴纳金额 from edu_teacher_partyfee group by 缴纳日期,机构名称,人员编号";
	ShowData();
	...
	{
		global $db;
		$sql = "select distinct Date_Format(创建时间,'%Y-%m') as 缴纳月份 from edu_teacher_partyfee order by 缴纳月份 desc ";
		$rs = $db->Execute($sql);
		$rs_a = $rs->GetArray();
	...
	function By_Organization($by_organization_day,$by_organization_month,$by_person_day,$by_person_month,$total_day,$Month_Pure,$Year_Pure)
	{
		...
		global $db;
		//判断总共有多少机构在本月有过操作
		$sql = "select distinct 机构名称 from edu_teacher_partyfee where Date_Format(缴纳时间,'%Y-%m')= '".$Year_Pure."-".$Month_Pure."'";
		$rs = $db->Execute($sql);
		$rs_a = $rs->GetArray();
				...
				for($j=0;$j<sizeof($编号);$j++)
				{
					for($day=1;$day<=$total_day;$day++)
					{
						 ...
						 if($day==1)
						{
							...
							$姓名 = returntablefield("edu_teacher_partymember","用户名",$编号[$j],"姓名");
	...
	function By_Year($by_year,$total_day,$Year_Pure)
	{
		...
		print "<Tr><Td class=TableContent colspan=1>".$Year_Pure."全年</Td>";
