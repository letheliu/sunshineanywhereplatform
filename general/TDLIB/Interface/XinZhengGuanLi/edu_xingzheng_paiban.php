...
<tr class="TableData">
<td>
&nbsp;<font color="red">当前学期:<?=$当前学期;?></font>&nbsp;<BR>
&nbsp;<font color="green">当前日期:<?=返回目标日期($当前周次,$当前星期)?></font>&nbsp;<BR>
&nbsp;<font color="green">当前周次:第<?=$当前周次;?>周</font>&nbsp;<BR>
&nbsp;<font color="green">当前星期:<?=$星期;?></font>&nbsp;<BR>
&nbsp;<font color="blue">当前班次:<?=$班次名称;?>(<?=$考勤时间段;?>)
</font></td>
</tr>
<tr class="TableHeader">
<td>
...
?>
<input type="hidden" name="考勤日期" value=<?=$考勤日期;?> />
<input type="hidden" name="当前学期" value=<?=$当前学期;?> />
<input type="hidden" name="当前周次" value=<?=$当前周次;?> />
<input type="hidden" name="当前星期" value=<?=$当前星期;?> />
<input type="hidden" name="班次名称" value=<?=$班次名称;?> />
</td>
</tr>
<tr class="TableData">
<td align="center">
...
<table width="100%" class="TableBlock" align="center">
<tr class="TableHeader">
<td colspan=8>设置排班情况&nbsp;当前周次:<?=$当前周次?>
&nbsp;时间范围: <?=substr($返回目标日期=返回目标日期($当前周次,1),6,11)?> 至 <?=substr($返回目标日期=返回目标日期($当前周次,7),6,11)?>
<input name='从上周获取数据' value='从第<?=($当前周次-1)?>周获取排班数据' class=SmallButton type=Button OnClick="location='?当前周次=<?=$当前周次?>&action=从上周获取数据'" <?=$disabled_上周?>>
<input name='从上周获取数据' value='从第<?=($当前周次-2)?>周获取排班数据' class=SmallButton type=Button OnClick="location='?当前周次=<?=$当前周次?>&action=从前周获取数据'" <?=$disabled_前周?>>
</td>
</tr>
<tr class="TableData" align="center">
	<td width="16%">班次\星期</td>
	<td width="12%">星期一<BR><?=返回目标日期($当前周次,1)?></td>
	<td width="12%">星期二<BR><?=返回目标日期($当前周次,2)?></td>
	<td width="12%">星期三<BR><?=返回目标日期($当前周次,3)?></td>
	<td width="12%">星期四<BR><?=返回目标日期($当前周次,4)?></td>
	<td width="12%">星期五<BR><?=返回目标日期($当前周次,5)?></td>
	<td width="12%">星期六<BR><?=返回目标日期($当前周次,6)?></td>
	<td width="12%">星期日<BR><?=返回目标日期($当前周次,7)?></td>
</tr>
<?
	$班次信息 = GetBanCiInfor();
