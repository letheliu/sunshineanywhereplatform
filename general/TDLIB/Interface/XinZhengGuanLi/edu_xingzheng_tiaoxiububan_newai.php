...
if($_GET['action']=="operation_tiaoxiutongguo"&&$_GET['selectid']!="")			{
	...
	for($i=0;$i<sizeof($Array);$i++)	{
		...
		if($Element!="")		{
			...
			if($审核状态!=1){
			$sql = "update edu_xingzheng_tiaoxiububan set 调休审核状态='1',调休审核人='$审核人',调休审核时间='$审核时间' where 编号='$Element'";
			$rs = $db->Execute($sql);
			$sql."<BR>"; 
			}
		}
...
if($_GET['action']=="operation_bubantongguo"&&$_GET['selectid']!="")			{
	...
	for($i=0;$i<sizeof($Array);$i++)	{
		...
		if($Element!="")		{
			$审核状态 = returntablefield("edu_xingzheng_tiaoxiububan","编号",$$Element,"审核状态");
			if($审核状态!=1){
			$sql = "update edu_xingzheng_tiaoxiububan set 调休审核状态='0',补班审核状态='1',补班审核人='$审核人',补班审核时间='$审核时间' where 编号='$Element'";
			$rs = $db->Execute($sql);
			$sql."<BR>"; 
			}
		}
