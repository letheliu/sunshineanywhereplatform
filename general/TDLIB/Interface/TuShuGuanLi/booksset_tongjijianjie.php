...
<TR><TD class=TableHeader align=left colSpan=40>&nbsp;固定资产信息按购买时间进行分科室分类别统计 <a href='booksset_tongji.php?开始时间=".$_GET['开始时间']."&结束时间=".$_GET['结束时间']."'>明细统计</a></TD></TR>
";
...
$SortArray = array();
//处理基本表数据
$sql = "select SUM(数量) AS 资产数量,SUM(数量*单价) AS 资产总金额,所属部门 from booksset where 购买日期>='$开始时间' and 购买日期<='$结束时间' and 图书名称!='' and 所属状态!='资产己报废' group by 所属部门";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
