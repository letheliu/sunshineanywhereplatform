...
for($i=0;$i<sizeof($rs_a);$i++)			{
	$学院代码 = $rs_a[$i]['学院代码'];
	$学院名称 = $rs_a[$i]['学院名称'];
	print "<TreeNode id=\"$学院代码\" text=\"[$学院名称]".$AddInfor."\" href=\"#\" target=\"_self\" img_src=\"images/node_user.gif\" title=\"$学院名称\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$学院代码&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=XI&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	...
	print "<TreeNode id=\"$DEPT_ID\" text=\"[$DEPT_NAME]".$AddInfor."\" href=\"#\" target=\"_self\" img_src=\"images/node_user.gif\" title=\"$DEPT_NAME\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$DEPT_ID&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=0&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
}
*/
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	...
	print "<TreeNode id=\"$系代码\" text=\"[$系名称]".$AddInfor."\"  href=\"#\" target=\"_self\"  img_src=\"images/node_user.gif\" title=\"$系名称\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$系代码&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=JIBIE&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	...
	print "<TreeNode id=\"$专业代码\" text=\"[$专业名称]".$AddInfor."\"  href=\"#\" target=\"_self\"  img_src=\"images/node_user.gif\" title=\"$专业名称\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$专业代码&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=BANJI&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
	//print "<TreeNode id=\"$USER_ID\" text=\"[$系名称]\" href=\"../Interface/$DIR/".$PARA_URL2."?班号=$系名称\" target=\"edu_main\" img_src=\"images/node_user.gif\" title=\"$系名称\"/>\r\n";
}
}
*/
...
if(($DEPT_PARENT!=""&&$PRIV_NO_FLAG=="JIBIE")||($SCHOOL_MODEL==3&&$PRIV_NO_FLAG=="")||($SCHOOL_MODEL<3&&$DEPT_PARENT!=""&&$PRIV_NO_FLAG=="JIBIE"))				{
	...
	for($i=0;$i<sizeof($rs_a);$i++)			{
		...
		$入学年份 = $rs_a[$i]['入学年份'];
		//href=\"../Interface/$DIR/edu_banji_newai.php?所属专业=$专业代码\" target=\"edu_main\"
		print "<TreeNode id=\"$入学年份\" text=\"[$入学年份]".$AddInfor."\"  href=\"#\" target=\"_self\"  img_src=\"images/node_user.gif\" title=\"$入学年份\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$DEPT_PARENT&amp;PARA_URL1=$入学年份&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=$ACTION_MODE_201003&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
	}
}
...
$datetime = date("Y-m-d");
	//模式201003
	$sql = "select distinct 入学年份,备注 from edu_banji where 所属系='$DEPT_PARENT' and 毕业时间>='".date("Y-m-d")."' and 入学年份='$入学年份' and 备注 like '$入学年份%' order by 入学年份 desc ,备注";
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
	//print $DIR;exit;
	for($i=0;$i<sizeof($rs_a);$i++)			{
...
$所属系 = $DEPT_PARENT;
$datetime = date("Y-m-d");
$sql = "select 班级代码,班级名称 from edu_banji where 所属系 = '$所属系'  and 入学年份 = '$入学年份'  and 毕业时间>='$datetime'  order by 班级名称";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
//print $DIR;exit;
for($i=0;$i<sizeof($rs_a);$i++)			{
	$班级代码 = $rs_a[$i]['班级代码'];
	$班级名称 = $rs_a[$i]['班级名称'];
	//print "<TreeNode id=\"$班级代码\" text=\"[$班级名称]".$AddInfor."\" href=\"../Interface/$DIR/".$PARA_URL2."?XZB=$班级名称\" target=\"edu_main\" img_src=\"images/node_user.gif\" title=\"$班级名称\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$班级代码&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=0&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	$学号 = $rs_a[$i]['学号'];
	$姓名 = $rs_a[$i]['姓名'];
	//print "<TreeNode id=\"$班级代码\" text=\"[$班级名称]".$AddInfor."\" href=\"../Interface/$DIR/".$PARA_URL2."?XZB=$班级名称\" target=\"edu_main\" img_src=\"images/node_user.gif\" title=\"$班级名称\" Xml=\"''.php?DEPT_ID=$班级代码&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=0&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
	//print "<TreeNode id=\"$班级代码\" text=\"[$班级名称][$班级代码]\" href=\"../Interface/$DIR/".$PARA_URL2."?".base64_encode("action=init_default&班号=$班级名称&pageid=1")."\" target=\"edu_main\" img_src=\"images/node_user.gif\" title=\"$班级名称\"/>\r\n";
	//http://localhost/general/EDU/Framework/''.php?DEPT_ID=04&amp;PARA_TARGET=&amp;PRIV_NO_FLAG=STUDENT&amp;PARA_URL1=2008&amp;PARA_URL2=电子0801班&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=EDU
...
//入学年份SELECT DISTINCT `入学年份` FROM `edu_banji`  LIMIT 0 , 30
$sql = "SELECT DISTINCT `入学年份` FROM `edu_banji` where 1=1  and 毕业时间>='".date("Y-m-d")."' order by 入学年份 desc";
$rs = $db->Execute($sql);
$rs_a = $rs->GetArray();
for($i=0;$i<sizeof($rs_a);$i++)			{
	$DEPT_ID = $rs_a[$i]['入学年份'];
	$DEPT_NAME = $rs_a[$i]['入学年份'];
	$NewYear = $DEPT_NAME+$学制信息;
	print "<TreeNode id=\"$DEPT_ID\" text=\"[$DEPT_NAME]".$AddInfor."\" href=\"#\" target=\"_self\" img_src=\"images/node_user.gif\" title=\"$DEPT_NAME\" Xml=\"inc_banjionly_tree.php?DEPT_ID=$DEPT_ID&amp;PARA_URL1=&amp;PARA_URL2=$PARA_URL2&amp;PARA_TARGET=$PARA_TARGET&amp;PRIV_NO_FLAG=BANJI&amp;PARA_ID=&amp;PARA_VALUE=&amp;MANAGE_FLAG=&amp;DIR=$DIR\"/>\r\n";
}
}
//级别
//列出班级
