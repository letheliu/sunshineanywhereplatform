...
$sql="select FUNC_ID_STR from user_priv2 where USER_PRIV='".$USER_PRIV."'";
...
$sql="select * from sys_function2 order by MENU_ID";
//print $sql;
//print_R($_GET);
$rs=$db->CacheExecute(150,$sql);
...
$sql="select * from sys_menu2 $whereSql order by MENU_ID";//print $sql;
...
function menu_table_3($showtext,$url,$MEMO,$pic,$tree_pic="tree_blank.gif",$tree_pic2="tree_line.gif",$tree_pic3='tree_line.gif')	{
	//if(!is_file($_SERVER['DOCUMENT_ROOT']."/images/menu/$pic"))		$pic = "2042.gif";
	$MEMO==""?$MEMO = $showtext : '';
...
function parent_table_1($showtext="我的办公桌",$pic="mytable.gif",$id="01",$image='tree_plus.gif')	{
	global $menu_mark;
	//if(!is_file($_SERVER['DOCUMENT_ROOT']."/images/menu/$pic"))		$pic = "2041.gif";
	print "<TABLE class=small cellSpacing=0 cellPadding=0 border=0>\n";
    print "<TBODY>\n";
    print "<TR>\n";
...
<?
if(is_dir($_SERVER['DOCUMENT_ROOT']."/module/word_model"))		{
