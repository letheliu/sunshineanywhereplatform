...
<?
	print "
		...
		<td align=center >评比人分组名称<BR></td>
		</tr>
	";
	$当前时间 = date("Y-m-d",mktime(1,1,1,7,30,date('Y')+1));
	$sql = "select distinct 组别名称 from ceping_renyuan_grop";
	//print $sql;
	$rs = $db->CacheExecute(150,$sql);
	$rs_a = $rs->GetArray();
	for($i=0;$i<sizeof($rs_a);$i++)		{
		$FieldValue = $rs_a[$i]['组别名称'];;
		print "
		<tr class=TableData>
		<td align=center onclick=\"javascript:parent.user.location='user.php?组别名称=$FieldValue&TO_ID=".$_GET['TO_ID']."&TO_NAME=".$_GET['TO_NAME']."&FORM_NAME=".$_GET['FORM_NAME']."';\" style=\"cursor:pointer\">".$FieldValue."</td>
		</tr>
		";
	}
