	...
	if(key!=kword.value && kword.value!="")
	{
     key=kword.value;
	   ...
	   if($('search_icon').src.indexOf("/images/quicksearch.gif")>=0)
	   {
	   	   ...
	   	   $('search_icon').onclick=function(){kword.value='按教材名称搜索...';$('search_icon').src="/images/quicksearch.gif";$('search_icon').title="";$('search_icon').onclick=null;};
	   }
  }
...
$出版社 = $_GET['出版社'];
if($出版社!="")		{
	$sql = "select 教材名称,教材编号 from edu_jiaocai where 出版社='".$出版社."' order by 教材名称";
}
else	{
	$sql = "select 教材名称,教材编号 from edu_jiaocai order by 教材名称";
}
...
for($i=0;$i<sizeof($rs_a);$i++)			{
	...
	echo "\r\n<tr class=\"TableData\">\r\n  <td class=\"menulines\" nowrap align=\"center\" onclick=\"javascript:add_user('";
	echo $教材编号;
	echo "','";
	echo "".$教材名称."";
