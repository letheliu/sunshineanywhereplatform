...
//session_start();
//session_register("SYSTEM_ALL_MODE");
...
if($_GET['MakeSystemModel']!="")			{
	...
	FormTextFile("config.php","<? \$SYSTEM_MODE_DIR = \"$SYSTEM_ALL_MODE\" ; ?>");
	echo "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=?Tablename=$Tablename&FileIniname=$FileIniname'>\n";
	exit;
...
print "<TITLE>Sunshine Anywhere 企业软件开发平台 集成开发环境</TITLE><link rel='stylesheet' type='text/css' href='/theme/9/style.css'>";
	...
	*/
	...
	for($i=0;$i<sizeof($NewArrayKeys);$i++)					{
		...
		if($fieldName_boolean)			{
			...
			if($fieldName_group)			{
				...
				//print $fieldName_filter_Search;exit;
	...
	if($section=="init_default"||$section=="init_customer"||$section=="init_default2")		{
		...
		if(sizeof($ADV_SEARCH_ARRAY)>2)			{
			...
			substr_replace($ArrayGroup['action_model'],",exportadv_default:exportadv:x",0,9999);
		}
	}
	...
	fclose($handle);
	//print "<BR><BR><BR><BR><div align=center><INPUT class=SmallButton onclick=\"location='?Tablename=".$_POST['Tablename']."'\" type=button value='回到目录'>　<INPUT class=SmallButton onclick=\"location='?Tablename=".$_POST['Tablename']."&sectionName=".$_POST['section']."'\" type=button value='继续配置'></div>";
	print "";
...
if($_GET['action']=="doUserInterfaceData"&&$_GET['Tablename']!=""&&$_GET['sectionName']!="")	{
	...
	//写入到管理员权限--开始
	$Insert_ID =="" ? $PRIV_ID = $rs->fields['FUNC_ID'] : $PRIV_ID = $Insert_ID;
	$PRIV_ID;
	$sql = "select FUNC_ID_STR from user_priv where user_priv='1'";
	$rs = $db->Execute($sql);
	...
	if(in_array($PRIV_ID,$FUNC_ID_STR_ARRAY))		{
		...
		$FUNC_ID_STR .= $PRIV_ID.",";
		$sql = "update user_priv set FUNC_ID_STR = '$FUNC_ID_STR' where USER_PRIV='1'";
		$rs = $db->Execute($sql);
	}
	//写入到管理员权限--结束
...
//从新增视图导入模块
//##############################################################################
$ACTION_ARRAY = array("initimportfromaddview","init2importfromaddview","view2importfromaddview","viewimportfromaddview","editimportfromaddview","reportimportfromaddview","statisticsimportfromaddview",);
